...
// creates adjacency matrix from csv file
...
// traverses the whole matrix and returns vector with distance frm each node to root
...
vector<int> bfs(int start, string filename, int size)
{
    ...
    /*for (unsigned i = 0; i <= 4038; i++) {
        if (d[i] >= 6) {
            std::cout << "distance from 0 to node " << i << ":" << d[i] << std::endl;
        }
    }*/
    return d;
}
...
// a* search depends on 1. edge weights and 2. node heuristics
...
void aStarSearch(vector<vector<int>> adj_mat, Pair src, Pair dest)
{
    ...
    if (isUnBlocked(adj_mat, src.first, src.second) == false
        || isUnBlocked(adj_mat, dest.first, dest.second)
               == false) {
